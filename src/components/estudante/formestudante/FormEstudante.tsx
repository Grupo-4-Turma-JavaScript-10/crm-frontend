...
export default function FormEstudante() {
  ...
  const estudanteInicial: Estudantes = {
    id: 0,
    nome: "",
    email: "",
    endereco: "",
    ...
    ativo: true,
    avatar: "adventurer",
    bolsa: { id: 0, nome: "" } as Bolsa,
  };

  const [estudante, setEstudante] = useState<Estudantes>(estudanteInicial);
  ...
  useEffect(() => {
    async function carregarDados() {
      try {
        ...
        if (id) {
          const dadosEstudante: Estudantes = await buscarDireto(`/estudante/${id}`, {});
          setEstudante(dadosEstudante);
        } else {
          setEstudante({
            ...estudanteInicial,
            bolsa: dadosBolsas[0] ?? estudanteInicial.bolsa,
          });
        }
      } catch (error) {
        console.error(error);
  ...
  function handleBolsaChange(e: ChangeEvent<HTMLSelectElement>) {
    const selecionada = bolsas.find((b) => b.id === Number(e.target.value)) ?? bolsas[0];
  ...
  async function salvar(e: FormEvent<HTMLFormElement>) {
    ...
    setIsLoading(true);
    try {
      const payload = { ...estudante, bolsaId: estudante.bolsa.id };
      if (id) {
    ...
    <div className="min-h-screen bg-gray-100 flex justify-center items-start py-10 px-4">
      <div className="bg-white w-full max-w-lg rounded-2xl shadow-xl p-8">
        <h1 className="text-3xl font-bold mb-6 text-gray-800">
          {id ? "Editar Estudante" : "Cadastrar Estudante"}
        </h1>
